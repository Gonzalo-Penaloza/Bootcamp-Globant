...
public class Matematica {
    ...
    public void calcularPotencia(){
        int base = (int) devolverMayor();
        int potencia = (int) devolverMenor();
        int resultado = (int) Math.pow(base,potencia);

        System.out.println(base + "^" + potencia + " es igual a: " + resultado);
    }

    public void calcularRaiz(){
        double radicando = Math.abs(devolverMenor());
        double raiz = Math.sqrt(radicando);

        System.out.println("La raiz cuadrada de "+radicando+" es: " + raiz);
    }
}
